...
def main() -> None:
    ...
    callback_fim = RFIM_callback(plot_path=artifact_dir / "rFIM_trace_10k.png")
    callbacks: list[BaseCallback] = [
        callback_return,
        callback_fim,
        change_env(10000, 10, (True, False)),
    ]
    ...
    # Save trained model
    model.save("ddpg_swimmer_no_optimize_10k_")

    # Save visualizations and data
    callback_return.plot_returns(artifact_dir / "mean_return_10k.png")

    mean_return_data: dict[str, Any] = {
        "episode_returns": callback_return.episode_returns,
        "mean_returns": callback_return.mean_returns,
    }
    with open(artifact_dir / "mean_return_data_10k.json", "w") as f:
    ...
    with open(artifact_dir / "rFIM_trace_data_10k.json", "w") as f:
